...
public final class MobConfigBogged {
	private static final String DEFAULT_MOB_DROPS = "minecraft-entities-bogged.json";

	private MobConfigBogged() {
	}
	...
	public static JsonObject buildDefaults() {
		...
		JsonObject bogged = new JsonObject();
		bogged.addProperty(MobConfigManager.FIELD_CUSTOM_MOB_DROPS, true);
		bogged.addProperty(MobConfigManager.FIELD_DIFFICULTY_SCALING, true);
		...
		bogged.add(MobConfigManager.FIELD_DIFFICULTY_SCALE, difficultyScale);

		JsonObject regionalDifficultyScale = new JsonObject();
		regionalDifficultyScale.addProperty(MobConfigManager.FIELD_ENABLED, true);
		bogged.add(MobConfigManager.FIELD_REGIONAL_DIFFICULTY_SCALING, regionalDifficultyScale);
		...
		defaultGroup.add(
			...
			buildBoggedMobStatsDefaults()
		);
		defaultGroup.add(
			MobConfigManager.FIELD_SPAWN_RULES,
			MobConfigManager.mobSpawnRules()
				.spawnWeight(90.0d)
				.equipmentSet(buildBoggedEquipmentSetDefaults())
				.build()
		);
		defaultGroup.add(
		...
		bogged.add(MobConfigManager.FIELD_DEFAULT_GROUP, defaultGroup);
		bogged.add("bogged-jockey", buildBoggedJockeyVariantDefaults());

		root.add(MobConfigManager.FILE_BOGGED, bogged);
		return root;
	}

	private static JsonObject buildBoggedMobStatsDefaults() {
	...
	private static JsonObject buildBoggedEquipmentSetDefaults() {
		JsonObject equipmentSet = new JsonObject();
		equipmentSet.addProperty(MobConfigManager.FIELD_ENABLED, true);
		equipmentSet.addProperty(MobConfigManager.FIELD_MOB_EQUIPMENT, "minecraft-equipment-bogged.json");
		equipmentSet.addProperty(MobConfigManager.FIELD_EQUIPMENT_CHANCE, 10.0d);
		return equipmentSet;
	}

	private static JsonObject buildBoggedJockeyVariantDefaults() {
		JsonObject variant = new JsonObject();
		variant.addProperty(MobConfigManager.FIELD_SHARED_COMPONENTS, true);

		JsonObject passenger = new JsonObject();
		JsonObject passengerMob = new JsonObject();
		passengerMob.addProperty(MobConfigManager.FIELD_ADULT_GROUP, "minecraft:bogged");

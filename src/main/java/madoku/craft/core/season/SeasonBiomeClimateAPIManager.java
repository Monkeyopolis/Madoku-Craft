...
public final class SeasonBiomeClimateAPIManager {
	public record Climate(double temperature, double humidity) { }
	private static volatile Map<Biome, BiomeClimateConfigAPIManager.Climate> runtimeConfiguredClimates = Map.of();
	private static volatile Map<Biome, Climate> runtimeNativeClimates = Map.of();
	private static volatile MinecraftServer currentServer;

	private SeasonBiomeClimateAPIManager() { }

	public static void initialize() {
		BiomeClimateConfigAPIManager.initialize();
	}
	...
	private static void rebuildConfiguredClimates(MinecraftServer server) {
		...
		IdentityHashMap<Biome, BiomeClimateConfigAPIManager.Climate> configured = new IdentityHashMap<>();
		...
		for (Map.Entry<ResourceKey<Biome>, Biome> entry : registry.entrySet()) {
			...
			BiomeClimateConfigAPIManager.Climate climate = BiomeClimateConfigAPIManager.getBiomeClimate(biomeId.toString());
			if (climate != null) {
	...
	public static boolean isTemperatureEnabled() {
		return BiomeClimateConfigAPIManager.getSettings().temperatureEnabled();
	}

	public static boolean isHumidityEnabled() {
		return BiomeClimateConfigAPIManager.getSettings().humidityEnabled();
	}
	...
	public static Climate resolve(ServerLevel level, net.minecraft.core.BlockPos pos) {
		...
		if (!SeasonAPIManager.isEnabled()) return nativeClimate(biome);
		return resolve(biome);
	}

	public static Climate resolve(Biome biome) {
		Climate nativeClimate = nativeClimate(biome);
		BiomeClimateConfigAPIManager.Climate configured = runtimeConfiguredClimates.get(biome);
		if (configured == null) {

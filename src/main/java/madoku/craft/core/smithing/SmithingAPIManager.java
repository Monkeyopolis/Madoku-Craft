...
public final class SmithingAPIManager {
	...
	public static void initialize() {
		SmithingConfigAPIManager.initialize();
	}

	public static void reset() {
		SmithingConfigAPIManager.reset();
	}

	public static void onServerStarted(MinecraftServer server) {
		SmithingConfigAPIManager.onServerStarted(server);
	}

	public static boolean acceptsPetItems() {
		return SmithingConfigAPIManager.isEnabled() && MadokuPetManager.isEnabled();
	}

	public static boolean acceptsExtendedItems() {
		return SmithingConfigAPIManager.isEnabled()
			&& (MadokuItemsManager.isEnabled() || MadokuPetManager.isEnabled());
	}
	...
	public static void applyCustomResult(SmithingMenu menu) {
		...
		if (duplicateUpgrade) {
			EnchantAPIManager.mergeEnchantments(base, additional, result);
		} else {
			EnchantAPIManager.copyEnchantments(base, result);
			// Netherite conversion is a new item tier, so it always starts at level 1.
			if (MadokuItemsManager.isEnabled() && MadokuItemsManager.isRarityCategoryItem(result)
				&& MadokuItemsManager.areItemLevelsEnabled()) {
				MadokuItemsManager.setItemLevel(result, 1);
			}
		}
	...
	private static Tier rarityOf(ItemStack stack) {
		return RarityAPIManager.detectAppliedRarity(stack);
	}

...
public final class MadokuSmithingManager {
	...
	public static boolean acceptsPetItems() { return SmithingConfigAPIManager.isEnabled() && MadokuPetManager.isEnabled(); }
	public static boolean acceptsExtendedItems() { return SmithingConfigAPIManager.isEnabled() && (MadokuItemsManager.isEnabled() || MadokuPetManager.isEnabled()); }
	...
	public static boolean isManagedBase(ItemStack stack) {
		...
		return (MadokuItemsManager.isEnabled() && MadokuItemsManager.isRarityCategoryItem(stack)) || (MadokuPetManager.isEnabled() && PetEntitiesManager.isPetItem(stack));
	}
	...
	private static boolean increaseLevel(ItemStack stack) {
		...
		if (MadokuPetManager.isEnabled() && PetEntitiesManager.isPetItem(stack)) { int current = PetEntitiesManager.petLevel(stack); if (current >= MadokuPetManager.maxPetLevel()) return false; PetEntitiesManager.setPetLevel(stack, current + 1); return true; }
		return false;
	}
	private static void copyManagedLevel(ItemStack source, ItemStack target) {
		...
		if (MadokuPetManager.isEnabled() && PetEntitiesManager.isPetItem(target)) PetEntitiesManager.setPetLevel(target, PetEntitiesManager.petLevel(source));
	}
	private static int levelOf(ItemStack stack) { return MadokuPetManager.isEnabled() && PetEntitiesManager.isPetItem(stack) ? PetEntitiesManager.petLevel(stack) : itemLevel(stack); }
